function [k, res_history] = run_SRGS_core(w1, w2, N_x, N_y, dx, U_init, B, max_iter, max_residual, check_interval, epsilon, track_residual)
%#codegen    
    U=U_init;
    U(2:N_x-1, 1) = B;

    diverged = false;
    
    k = int32(max_iter);
    cycle=int32(max_iter);
    
    res_history = 0;
    inv_dx2 = 1/(dx^2);
    res=0;
    res_sum=0;

    if track_residual
        res_history = zeros(int32(max_iter),1);
    end

    num_cycles = int32(max_iter / 2);

    for cycle = int32(1):int32(num_cycles)
        k_odd = 2*cycle-1;
        k_even = 2*cycle;

        res_sum=0;
        
        
        for j = 2:N_y-1
            for i = 2:N_x-1
                U_GS = 0.25 * (U(i+1,j) + U(i-1,j) + U(i,j+1) + U(i,j-1));
                U(i,j) = (1 - w1) * U(i,j) + w1 * U_GS;
            end
        end

        for j = 2:N_y-1
            for i = 2:N_x-1
                U_GS = 0.25 * (U(i+1,j) + U(i-1,j) + U(i,j+1) + U(i,j-1));
                U(i,j) = (1 - w2) * U(i,j) + w2 * U_GS;
            end
        end
        
        if (int32((mod(k_even, check_interval))) == int32(0))||(cycle==1)
            res_sum=0;
            for j = 2:N_y-1
                for i = 2:N_x-1
                    res = (U(i+1,j) + U(i-1,j) + U(i,j+1) + U(i,j-1) - 4 * U(i,j)) * inv_dx2;
                    res_sum = res_sum + abs(res);
                end
            end

            if track_residual
                res_history(k_odd)=res_sum;
                res_history(k_even)=res_sum;
            end
            if res_sum<epsilon 
                k = k_even;
                return;
            end
            
            if isnan(res_sum) || res_sum > max_residual
                diverged = true;
                break
            end

        elseif track_residual
            if k_odd>1
                res_history(k_odd)=res_history(k_odd-1);
                res_history(k_even)=res_history(k_odd);

            end
        end

        
        
    end
    if diverged
        k = max_iter;
    end
end