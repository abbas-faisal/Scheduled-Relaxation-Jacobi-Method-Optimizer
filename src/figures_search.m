...
GS_sim = load(fullpath,'iters','res_history');
...
for d=1:resolution
    ...
    if d==1
        ...
        plot(1.0, 1.0, 'w*', 'MarkerSize', 12, 'LineWidth', 1.75, ...
...
%hold on;
%semilogy(1:length(GS_sim.res_history), GS_sim.res_history, ...
%    'b-', 'LineWidth', 2, ...
%    'DisplayName', sprintf('Gauss Siedel Method')...
%    )
hold on;
semilogy(1:length(SRJ_sim.res_history), SRJ_sim.res_history, ...
    'b-', 'LineWidth', 2, ...
    'DisplayName', sprintf('Calculated Optimal SRJ (\\omega_1=%.4f, \\omega_2=%.4f)', SRJ_sim.best_w1, SRJ_sim.best_w2)...
    )
semilogy(1:length(SRJ_sim.res_history_theory), SRJ_sim.res_history_theory, ...
    'g-', 'LineWidth', 2, ...
    'DisplayName', sprintf('Theoretical Optimal SRJ (\\omega_1=%.4f, \\omega_2=%.4f)', omega_1_theory, omega_2_theory)...
    )
