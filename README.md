...
This approximation has error $O(\Delta x,\Delta y)$. We implement the Point Jacobi method using a 100x100 mesh. This means $N_x=N_y=100$ with $\Delta x = \Delta y = (2\pi)/100$. This can be implemented with the following finite difference equation:
...
$$
\Omega_1^d = \{a\Delta \omega^d \mid a \in \mathcal{N},a\Delta \omega^d \in
[\omega_1^{d-1}-10W\Delta\omega^d,\omega_1^{d-1}+10W\Delta\omega^d]\} $$

$$
\Omega_2^d = \{b\Delta \omega^d \mid b \in \mathcal{N}, b\Delta \omega^d \in
...
We will also conduct a fine search around $\omega_2=\omega_2=1$. The intent of this search is to determine how small changes in the relaxation parameter around the initial Jacobi method can affect convergence behavior.
